"""``uv run cannet-local-sidecar`` entry point.
...
- The default tree (root + per-module ``_log = logging.getLogger(__name__)``)
  ...
  ``sidecar:python-can``.
- The ``cannet_local_sidecar.banner`` logger writes machine-parseable,
...
BANNER = logging.getLogger("cannet_local_sidecar.banner")
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"cannet-local-sidecar {__version__}",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        logging.getLogger("cannet_local_sidecar").error(
