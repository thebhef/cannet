...
pub(crate) struct AppState {
    ...
    /// The open project's bus `(id, name)` pairs.
    ///
    /// A math pattern is evaluated against the canonical signal path
    /// (ADR 0038), whose first segment is the bus *name*, so the host
    /// needs to know what the project calls its buses before it serves
    /// anything — including the first serve after an open, which no
    /// command of the math surface has necessarily preceded. Written by
    /// [`Self::set_project_bus_names`] from the two places the bus list
    /// reaches the host: the project-open parse, and the frontend's push
    /// on every bus add / rename / remove.
    pub(crate) project_bus_names: Mutex<Vec<(String, String)>>,
}
...
impl AppState {
    ...
    pub(crate) fn project_bus_names(&self) -> MutexGuard<'_, Vec<(String, String)>> {
        self.project_bus_names
            .lock()
            .expect("project_bus_names mutex poisoned")
    }

    /// Install the project's bus `(id, name)` pairs, dropping the
    /// resolved math model when they moved: a pattern's membership is
    /// judged against the names, so a rename can add or remove members.
    pub(crate) fn set_project_bus_names(&self, names: Vec<(String, String)>) {
        let mut standing = self.project_bus_names();
        if *standing == names {
            return;
        }
        *standing = names;
    ...
    pub(crate) fn math_model(&self, dbcs: &[LoadedDbc]) -> Arc<crate::math_signals::MathModel> {
        ...
        let bus_names: HashMap<String, String> = self.project_bus_names().iter().cloned().collect();
