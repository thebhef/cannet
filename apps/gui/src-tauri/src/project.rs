...
/// The `(id, name)` pairs the host keeps standing for the open
/// project's buses ([`crate::app_state::AppState::project_bus_names`]).
pub(crate) fn bus_name_pairs(buses: &[Bus]) -> Vec<(String, String)> {
    buses
        .iter()
    ...
    match parse_project(&text) {
        Ok(p) => {
            ...
            *state.math_model_cache() = None;
            // And with them the bus names their patterns resolve
            // against (ADR 0038): installed here, from the file just
            // parsed, so the first serve after an open already matches
            // on the names the user sees rather than on bus ids.
            state.set_project_bus_names(bus_name_pairs(&p.buses));
...
pub fn close_project(app: tauri::AppHandle, state: tauri::State<'_, crate::app_state::AppState>) {
    ...
    *state.signal_units() = std::sync::Arc::default();
    ...
    // and so do the bus names their patterns resolved against.
