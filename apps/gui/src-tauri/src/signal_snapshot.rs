...
///
/// `latest` is index-parallel with `resolved` — what each series'
/// filled pyramid says about itself, from
...
pub fn select_math(
    ...
    latest: &[Option<crate::signal_cache::MathLatest>],
    source_buses: Option<&[String]>,
    ...
    resolved
        ...
        .map(|(i, r)| {
            ...
            SignalSnapshotRecord {
                ...
                raw_field: false,
                display_hex: false,
                value: newest.map(|l| l.newest.value),
                // A computed value is physical throughout: there are no
                // raw bits behind it and no `VAL_` table over it.
                raw: None,
                label: None,
                // A math series' cadence comes from its operands — the
                // fold lays its samples on the union of their timelines
                // — which makes it a fact about the filled series, and
                // the cache measures it there like any other
                // provenance's. A capture-constant reports none.
                rate: newest.and_then(|l| l.rate),
                count: newest.map(|l| u64::try_from(l.count).unwrap_or(u64::MAX)),
                time_seconds: newest.map(|l| l.newest.t_seconds),
                // Stamped by `arrange_sections`, which runs next.
                section: None,
                file_backed: false,
                math: true,
...
mod tests {
    ...
    fn a_math_signal_is_a_row_of_a_signal_view() {
        ...
        let latest = vec![Some(crate::signal_cache::MathLatest {
            newest: crate::signal_sampler::SamplePoint {
                t_seconds: 4.5,
                value: 1.25,
            },
            count: 17,
        ...
        // A computed value is physical throughout — no raw bits behind
        // it — but it does have a cadence, the one its operands' union
        // timeline gave the filled series, and the row carries it.
        assert_eq!(row.raw, None);
        assert_eq!(row.rate, Some(9.5));
    }
