...
pub struct MathOperands {
    #[serde(default)]
    pub picks: Vec<Option<MathOperand>>,
...
impl MathOperands {
    ...
    pub fn filled(&self) -> impl Iterator<Item = &MathOperand> {
...
impl MathModel {
    ...
    pub fn resolve(
        ...
        for definition in definitions {
            ...
            // its own and takes the conversion alone. Membership is the
            // *filled* slots: an empty one names nothing, so it
            // contributes no operand, no scaling and no unit.
            let mut picked: Vec<MathOperand> = definition.operands.filled().cloned().collect();
...
mod tests {
    ...
    fn a_pairs_empty_slot_survives_the_wire() {
        ...
        assert_eq!(operands.picks, vec![None, Some(sig("B").into())]);
    ...
    fn a_pick_list_written_before_slots_still_fills_them_in_order() {
        ...
        assert_eq!(
            operands.picks,
            vec![Some(sig("A").into()), Some(sig("B").into())]
        );
        assert_eq!(serde_json::to_string(&operands).expect("serialize"), raw);
    }
    ...
    fn a_pair_with_an_empty_slot_is_not_usable_yet() {
        ...
        let d = def(
            ...
            MathOperands {
                picks: vec![None, Some(sig("B").into())],
                patterns: Vec::new(),
            },
        );
        assert!(
    ...
    fn a_sets_membership_is_picks_plus_live_pattern_matches() {
        let definitions = vec![def("m1", MathFunction::Max, {
            ...
            o.picks = vec![Some(MathOperand::new(sig("PackVolts")))];
            o
        })];
        let catalog = [
            entry("Cell01", "V"),
    ...
    fn a_source_unit_override_converts_a_mislabelled_operand() {
        ...
        d.unit = Some("A".into());
        d.operands.picks[0].as_mut().unwrap().source_unit = Some("milliampere".to_string());
    ...
    fn manual_scalars_work_with_no_units_anywhere() {
        ...
        d.operands.picks[0].as_mut().unwrap().gain = Some(2.0);
        d.operands.picks[0].as_mut().unwrap().offset = Some(-1.0);
    ...
    fn a_manual_scalar_composes_ahead_of_the_conversion() {
        let mut d = def("m1", MathFunction::Sum, picks(&[sig("Cell")]));
        d.unit = Some("A".into());
        d.operands.picks[0].as_mut().unwrap().gain = Some(0.5);
    ...
    fn a_definition_written_before_scaling_existed_still_loads() {
        ...
        let pick = d.operands.picks[0].as_ref().expect("slot A");
        assert_eq!(pick.reference.signal_name, "A");
        assert_eq!(pick.reference.bus_id.as_deref(), Some("b"));
        assert_eq!(pick.gain, None);
    ...
    fn a_scaled_definition_round_trips_through_json() {
        ...
        d.operands.picks[0].as_mut().unwrap().gain = Some(0.25);
        d.operands.picks[0].as_mut().unwrap().source_unit = Some("milliampere".to_string());
