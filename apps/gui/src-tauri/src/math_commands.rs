...
//! Every mutator announces through [`RegistryChangeSink`] — the
//! listing moved, and so did what the capture decodes to — and the
//! views refetch.
//!
//! **A command carries a definition and nothing else.** A pattern is
//! evaluated against the canonical signal path (ADR 0038), whose first
//! segment is the bus *name*, but the project's bus list is standing
//! host state ([`AppState::set_project_bus_names`]) written where that
//! list already arrives — so a name-anchored pattern resolves with no
//! math command having run, and there is nothing for a caller to pass.
...
fn with_default_name(mut definition: MathDefinition) -> MathDefinition {
    if definition.name.trim().is_empty() {
        ...
        {
            if definition.operands.filled().next().is_none() {
...
pub(crate) fn list_math_signals(state: State<'_, AppState>) -> Vec<MathSignalRecord> {
    ...
    model
        ...
        .map(|resolved| {
            ...
            candidate.operands.picks = resolved.operands.iter().cloned().map(Some).collect();
            MathSignalRecord {
...
#[tauri::command]
#[allow(clippy::needless_pass_by_value)]
pub(crate) fn define_math_signal(app: AppHandle, definition: MathDefinition) -> Result<(), String> {
...
#[tauri::command]
#[allow(clippy::needless_pass_by_value)]
pub(crate) fn update_math_signal(app: AppHandle, definition: MathDefinition) -> Result<(), String> {
    let state: State<'_, AppState> = app.state();
    state
        .math
...
/// What a write to the registry announces, as a seam.
///
/// A definition change is **two** facts with two audiences, and sending
/// only the first is what left a redefined series drawing its old
/// numbers:
///
/// - the **listing** moved — the editors, the Computed branch and a
///   plot's side list re-read it ([`MATH_SIGNALS_CHANGED`]);
/// - **what the capture decodes to** moved. A math series is the fourth
///   decode provenance, so a redefinition changes the decoded model
///   exactly as a DBC edit or a per-signal unit reinterpretation does,
///   and is announced the same way
///   ([ADR 0053](../../../docs/adr/0053-reload-when-it-applies-and-what-it-tells.md)
///   §2). That announcement is the only carrier the frontend's
///   re-anchor epoch has, and a plot's samples are cached against the
///   epoch — not against the listing.
///
/// A trait rather than the [`AppHandle`] itself because Tauri's
/// `Emitter` is sealed: this is the seam a test has on the rule, and
/// the rule going untested is why the missing announcement shipped.
pub(crate) trait RegistryChangeSink {
    /// Tell the surfaces that read the definition listing.
    fn math_signals_changed(&self);
    /// Announce a change to what the capture decodes to (ADR 0053 §2).
    fn decode_model_changed(&self);
}

impl RegistryChangeSink for AppHandle {
    fn math_signals_changed(&self) {
        let _ = self.emit(MATH_SIGNALS_CHANGED, ());
    }
    fn decode_model_changed(&self) {
        crate::dbc_commands::announce_dbc_change(self, "*");
    }
}

/// Drop the resolved model and tell the views ([`RegistryChangeSink`]
/// for what "the views" means — there are two sets of them).
///
...
pub(crate) fn changed<S: RegistryChangeSink>(sink: &S, state: &AppState) {
    *state.math_model_cache() = None;
    crate::app_state::invalidate_derived_caches(state);
    sink.math_signals_changed();
    sink.decode_model_changed();
}
    ...
    fn record() -> MathSignalRecord {
        let definition = MathDefinition {
            ...
            operands: MathOperands {
                picks: vec![Some(MathOperandRef::dbc("bus-a", 0x120, false, "Cell01"))],
