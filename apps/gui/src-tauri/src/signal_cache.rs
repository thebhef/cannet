...
/// One **math** series as a signal view's row reads it
/// ([`SignalCacheStore::math_latest`]): its newest sample and the two
/// statistics beside it. Read off the filled pyramid here rather than
/// re-derived by a caller — statistics are the model's job (ADR 0025),
/// as they already are for the file-backed [`FileSignalEntry`].
#[derive(Clone, Copy)]
pub struct MathLatest {
    /// Time and value of the newest sample.
    pub newest: SamplePoint,
    /// How many samples the series holds.
    pub count: usize,
    /// Samples per second over the series' own span: the cadence its
    /// operands gave it, since the fold lays every computed sample on
    /// the union of the operands' timelines.
    ///
    /// `None` where there is no cadence to report — fewer than two
    /// samples, or a **capture-constant** (an `hline`, a `statistic`),
    /// which is two points spanning the capture rather than a series
    /// sampled over it.
    pub rate: Option<f64>,
}
...
impl SignalCacheStore {
    ...
    /// What a *row* of a signal view needs about each math series in
    /// `queries` ([`MathLatest`]), as against the window a plot asks
    /// for.
    ///
    ...
    pub fn math_latest(
        ...
        keys.iter()
            .map(|key| {
                ...
                cache.read = true;
                let newest = cache.latest()?;
                // A user-authored constant is two points spanning the
                // capture, not a series sampled at `2/span` Hz.
                let constant = cache.math.as_ref().is_some_and(|m| m.constant);
                Some(MathLatest {
                    newest,
                    count: cache.sample_count(),
                    rate: if constant { None } else { cache.rate() },
                })
            })
            .collect()
    }
...
mod tests {
    ...
    fn a_statistic_defined_mid_capture_is_one_flat_line_that_moves() {
        // The order the bench hits: the capture is already running with
        // its operand on a plot — decoded over serve after serve — and
        // only *then* is the statistic defined, after which the capture
        // keeps growing under it. Every serve from that point on has to
        // answer with the one horizontal line the function names,
        // spanning the whole operand at the statistic over the whole
        // operand: a flat line whose value moves, never a curve.
        let db = dbc_ab(10);
        let definitions = vec![math_def(
            "m1",
            MathFunction::Statistic {
        ...
        let before = on_test_bus(&[&db]);
        let dbs = with_math(&db, &definitions);
        let dir = TempDir::new().unwrap();
        let store = SignalCacheStore::new_unbounded(dir.path());
        let growing = TraceStore::new();

        // Phase one: no math in the model at all, `A` served each round.
        for chunk in 0..3u64 {
            for i in chunk * 30..(chunk + 1) * 30 {
                growing.append(ab_frame(i * S, i as u16, (i * 10) as u16));
            }
        ...
        // Phase two: the definition exists, and the capture grows on.
        for chunk in 3..6u64 {
            for i in chunk * 30..(chunk + 1) * 30 {
                growing.append(ab_frame(i * S, i as u16, (i * 10) as u16));
            }
            let newest = ((chunk + 1) * 30 - 1) as f64;
            let series = math_series(&store, "m1", &growing, &dbs);
            assert_eq!(series.len(), 2, "chunk {chunk}: {series:?}");
            assert_eq!(series[0], (0.0, newest), "chunk {chunk}: {series:?}");
            assert_eq!(series[1], (newest, newest), "chunk {chunk}: {series:?}");
            assert_eq!(
                store.min_max_many(&[math_query("m1")], &growing, &dbs),
                vec![Some((newest, newest))],
                "chunk {chunk}",
            );
        }
    }
    ...
    fn the_latest_of_a_math_series_drives_its_own_fill() {
        ...
        let newest = latest[0].expect("the fill produced samples");
        assert_eq!(newest.count, 50);
        let whole = math_series(&store, "m1", &trace, &dbs);
        assert_eq!(
            (newest.newest.t_seconds, newest.newest.value),
            *whole.last().unwrap()
        );
    }

    /// A math row's `msg/s` column. The cadence is the operands' — the
    /// fold lays the computed samples on the union of their timelines —
    /// but that makes it a fact about the *filled series*, which the
    /// cache already measures for every other provenance. So the host
    /// answers it beside the newest sample rather than leaving the
    /// column blank for the frontend to guess at.
    ///
    /// A **capture-constant** is the exception: an `hline` or a
    /// `statistic` is two points spanning the capture ([`MathFill`]'s
    /// `constant`), so a samples-per-second reading off them would
    /// report `2/span` Hz for something that is not sampled at all.
    #[test]
    fn a_math_rows_rate_is_the_cadence_its_operands_gave_it() {
        // 50 frames one second apart, carrying both operands.
        let trace = ab_capture(50);
        let db = dbc_ab(10);
        let definitions = vec![
            math_def("m1", MathFunction::Sum, &[operand("A"), operand("B")]),
            math_def("line", MathFunction::HLine { value: 7.5 }, &[]),
        ];
        let dbs = with_math(&db, &definitions);
        let dir = TempDir::new().unwrap();
        ...
        let latest = store.math_latest(&[math_query("m1"), math_query("line")], &trace, &dbs);
        let sum = latest[0].expect("the fill produced samples");
        assert_eq!(sum.rate, Some(1.0));
        let constant = latest[1].expect("a constant is two points spanning the capture");
        assert_eq!(constant.rate, None);
    }
    ...
    fn a_manual_operand_scalar_reaches_the_samples() {
        ...
        definitions[0].operands.picks[0].as_mut().unwrap().gain = Some(3.0);
        definitions[0].operands.picks[0].as_mut().unwrap().offset = Some(-1.0);
        let dbs = with_math(&db, &definitions);
