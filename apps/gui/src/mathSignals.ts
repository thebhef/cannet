...
/// What fills a fixed-arity section's slot, or `null` where it stands
/// empty. The slot is a **position**: slot 1 is B whether or not A
/// holds anything.
export function pickInSlot(
  definition: MathDefinition,
  slot: number,
): MathOperandRef | null {
  return definition.operands.picks[slot] ?? null;
}

/// One operand placed in a section.
///
/// A single-signal slot replaces what it holds; the set section
/// appends, ignoring a signal it already collects. **A pick made into B
/// lands in B**: the slots before it stand empty rather than closing
/// up, so filling the second slot of a pair first does not make that
/// operand A.
export function withPick(
  ...
  if (slot === "set") {
    ...
    if (picks.some((p) => p != null && operandRefKey(p) === key)) return definition;
    picks.push(ref);
  } else {
...
///
/// A set closes up — its picks are a membership, and order is only the
/// order the user built it in. A fixed-arity slot is left **empty**
/// instead, because the operands after it are named: clearing A must
/// not promote B. Trailing empties are dropped, so a definition whose
/// slots fill from the front stores the dense list it always did.
export function withoutPick(definition: MathDefinition, index: number): MathDefinition {
  const picks = [...definition.operands.picks];
  if (mathFunctionSpec(definition.function.kind).arity === "set") {
    picks.splice(index, 1);
  } else {
    picks[index] = null;
    while (picks.length > 0 && picks[picks.length - 1] == null) picks.pop();
  }
  return { ...definition, operands: { ...definition.operands, picks } };
}
...
export function withOperandScaling(
  ...
  if (before == null) return definition;
...
export function setValidity(
  picks: readonly (MathOperandRef | null)[],
  patterns: readonly { valid: boolean; matches: number }[],
): SectionValidity {
  if (patterns.some((p) => !p.valid)) return { ok: false, message: "bad regex" };
  // A set closes up as it is edited, so it carries no empty slot; the
  // count is of what is actually there either way.
  const n =
    picks.filter((p) => p != null).length +
    patterns.reduce((sum, p) => sum + p.matches, 0);
  return n >= 2
