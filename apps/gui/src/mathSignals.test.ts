...
describe("one field's commit", () => {
  ...
  it("keeps a pair's pick in the slot it was made in", () => {
    // The slots are slots: B holds what was picked into B, and A stays
    // empty until something is picked into it.
    const pair = newMathDefinition("difference", "m1");
    const b = withPick(pair, 1, dbcRef("Cell02"));
    expect(b.operands.picks).toEqual([null, dbcRef("Cell02")]);
    const both = withPick(b, 0, dbcRef("Cell01"));
    expect(both.operands.picks).toEqual([dbcRef("Cell01"), dbcRef("Cell02")]);
    // Clearing A leaves the hole rather than sliding B into it.
    expect(withoutPick(both, 0).operands.picks).toEqual([null, dbcRef("Cell02")]);
    // Clearing B leaves the dense list a full definition always wrote —
    // a trailing empty slot is not stored.
    expect(withoutPick(both, 1).operands.picks).toEqual([dbcRef("Cell01")]);
  });
