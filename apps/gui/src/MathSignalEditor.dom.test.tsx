...
describe("filling a section", () => {
  ...
  it("leaves a pick made into B in B, with A still empty", async () => {
    // The slots are slots: filling the second one does not slide the
    // operand into the first, and the section headers still read the
    // way the user left them.
    renderEditor(pairRecord());
    fireEvent.drop(section("B"), {
      dataTransfer: fakeDataTransfer(signalPayload("Cell02")),
    });
    await waitFor(() => expect(lastWrite()).toBeDefined());
    expect(writtenDefinition().operands.picks).toEqual([null, dbcRef("Cell02")]);
  });

  it("shows a filled B beside an empty A", () => {
    renderEditor(
      mathRecord({
        kind: "difference",
        arity: "pair",
        function: { kind: "difference" },
        operands: { picks: [null, dbcRef("Cell02")], patterns: [] },
        resolvedOperands: [dbcRef("Cell02")],
        operandPaths: ["CAN1/BMS/BMS_Cells/Cell02"],
      }),
    );
    expect(section("A")).toHaveTextContent("pick one");
    expect(section("B")).toHaveTextContent("Cell02");
    expect(section("B")).toHaveTextContent("1 signal");
  });
...
describe("what the host says", () => {
  ...
  it("writes the definition and nothing else", async () => {
    renderEditor(mathRecord({ kind: "sum", function: { kind: "sum" } }));
    fireEvent.drop(section("Signals"), {
      dataTransfer: fakeDataTransfer(signalPayload("Cell01")),
    });
    // The host holds the project's bus list itself, so a write carries
    // the definition alone.
    await waitFor(() => expect(lastWrite()).toBeDefined());
    expect(Object.keys(lastWrite()!.args)).toEqual(["definition"]);
  });
