...
export function DatabasePanel(props: IDockviewPanelProps) {
  ...
  // `buses` is a trigger of its own: a pattern is evaluated against the
  // canonical path (ADR 0038), whose first segment is the bus *name*,
  // so a rename moves what a pattern selects.
  useEffect(() => refreshMath(), [buses, dbcPaths, dbcGeneration, refreshMath]);
  useEffect(() => {
    const unlisten = Promise.all([
  ...
  const handleCreateMath = useCallback(
    (kind: MathFunctionKind) => {
      ...
      recordPanelEdit({
        ...
        redo: [{ kind: "mathDefine", definition }],
      });
      void invoke("define_math_signal", { definition })
        .then(() => {
    ...
    [recordPanelEdit, refreshMath, setRowExpanded],
  );
  ...
  const handleMathDelete = useCallback(
    ...
    [mathSignals, recordPanelEdit, refreshMath],
  );
