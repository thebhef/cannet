...
function SignalRow({
  ...
  const cell = (column: SignalColumnKey): React.ReactNode => {
    ...
    switch (column) {
      case "bus":
        // A math signal (`docs/CONTEXT.md`) is bound to no bus of its
        // own, but its operands are: this column names the buses
        // feeding it, which is what a bus column names. The
        // *message-line* label ("Math - Multiple Busses") and the chips
        // that go with it belong to the message cell, below.
        if (math) return math.busIds.map((id) => busDisplayName(id, lookup)).join(", ");
        return busDisplayName(row.bus_id, lookup);
      case "ecu":
        return row.transmitter ?? "";
      case "msg":
        // This is the table's **provenance cell**: the one that says
        // where a row comes from when its bus / ECU / message columns
        // cannot (the file-backed badge below is the other case). It is
        // the direct analogue of a plot side list's message line, so a
        // math row carries exactly what that line carries — one color
        // chip per bus feeding it, transitively as the host resolved
        // them (ADR 0025), and the label that says so.
        if (math) {
          return (
            <>
