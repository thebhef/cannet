...
describe("the Computed branch", () => {
  ...
  it("asks the host for the listing through the shared provider", async () => {
    renderPanel();
    await waitFor(() =>
      expect(invoked.some((c) => c.cmd === "list_math_signals")).toBe(true),
    );
    // The panel no longer fetches the listing itself — the provider
    // every surface reads does, and it carries nothing: the project's
    // bus list is standing host state, so the call has no argument at
    // all.
...
describe("deleting one", () => {
  it("takes one click, and the undo step carries the definition back", async () => {
    ...
    await core.invoke("define_math_signal", { definition: back.definition });
    emitHostEvent("math-signals-changed");
    expect(await screen.findByText("median(Cell\d+)")).toBeInTheDocument();
  });
