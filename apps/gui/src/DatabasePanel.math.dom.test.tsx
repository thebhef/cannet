...
describe("the Computed branch", () => {
  ...
  it("asks the host for the listing, with nothing to tell it about the project", async () => {
    renderPanel();
    await waitFor(() =>
      expect(invoked.some((c) => c.cmd === "list_math_signals")).toBe(true),
    );
    // The host holds the project's bus list itself, so the listing is a
    // bare read — a pattern anchored on a bus name resolves without the
    // caller saying what the buses are called.
