...
export function MathSignalEditor({ record, definitions }: MathSignalEditorProps) {
  ...
  const commit = (next: MathDefinition) => {
    setError(null);
    recordPanelEdit({
      undo: [{ kind: "mathUpdate", definition: stored }],
      redo: [{ kind: "mathUpdate", definition: next }],
    });
    void invoke("update_math_signal", { definition: next }).catch((e: unknown) =>
      setError(String(e)),
    );
  };
  ...
  const picked = new Set(
    stored.operands.picks.filter((p) => p != null).map(operandRefKey),
  );
        ...
        <OperandSectionView
          ...
          validity={
            section.slot === "set"
              ...
              : slotValidity(pickInSlot(stored, section.slot))
          }
          describe={describe}
...
function OperandSectionView({
  ...
  // A fixed-arity section renders **its own slot** and nothing else, so
  // an empty slot before it is a gap in the form rather than a row that
  // slides up.
  const slotPick = isSet ? null : pickInSlot(definition, section.slot as number);
  const filled = isSet
    ? picks.flatMap((ref, index) => (ref == null ? [] : [{ ref, index }]))
    : slotPick == null
      ? []
      : [{ ref: slotPick, index: section.slot as number }];
...
function OperandScalingRow({
  ...
  label: string;
  operand: MathOperand | null | undefined;
  sourceUnitOptions: readonly ComboboxOption[];
