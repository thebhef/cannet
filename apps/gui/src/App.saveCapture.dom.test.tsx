...
/// remembered folder and format seed the picker. `folder` is optional
/// because that is the real wire shape: the host skips a `None` folder
...
describe("Export capture", () => {
  it("opens the picker before any export has remembered a folder", async () => {
    // A fresh install's host omits `folder` from `get_export_state`
    // (serde skips a `None`), so it arrives as `undefined`, not `null`.
    // The picker must still open, seeded with just the file name.
    exportState = { format: "blf", nameTemplate: "{project}-{start}" };
    await saveThrough(null);
    expect(saveOptions[0].defaultPath).toBe("demo-20260905T091502-0600.blf");
  });
