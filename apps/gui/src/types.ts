...
/// with. `folder` is unset until the first export — the host *omits* a
/// `None` folder rather than sending `null` — which leaves the OS
...
export interface MathOperands {
  /// A fixed-arity function's picks are **slots** — A and B for a pair
