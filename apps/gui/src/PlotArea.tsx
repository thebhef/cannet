...
export const PlotArea = memo(function PlotArea(p: PlotAreaProps) {
    ...
    try {
      ...
      if (outcome.extra) {
        ...
        signals.forEach((s, i) => {
          ...
          // Cached in the unit the signal *decodes* in, which is the
          // unit the host answered in. The carry into the drawn unit
          // happens where the range is read, below: this answer outlives
          // the display choice that was in force when it arrived, and an
          // extent already carried through one factor cannot be carried
          // through the next.
      ...
      signals.forEach((s, i) => {
        ...
        if (lr.followLive) {
          ...
          // converted series' all-time extent goes through the very same
          // affine its values do.
          if (e) {
            ranges.set(
              key,
              convertExtent({ lo: e.lo, hi: e.hi }, displayAffineOf(displayUnitsRef.current, key)),
            );
          }
          return;
        }
      ...
      // Where each series' markers may go — computed from the raw
      // timestamps against the same `xs` the rows were built on, before
      // any blanking or normalisation touches them, so it describes the
      // samples rather than whatever the merge made of them.
      sampleColumnsRef.current = sampleColumns(xs, seriesRel);
  ...
  useEffect(() => {
    ...
    // Where those markers may land, in every mode: the series' own
    // sample columns, never the ones the merge held it across (ADR
    // 0026). Read per draw for the same reason the floor is — a fetch
    // changes the columns without rebuilding the instance.
    applySampleMarkerFilter(u.series, (seriesIdx) => sampleColumnsRef.current[seriesIdx - 1] ?? []);
  ...
  // Re-draw through the factor when a series' display unit moves
  // (ADR 0026 — converting is a real conversion, so the values drawn,
  // the readout and the axis's extent all go through the affine). The
  // host serves the same raw values either way, so this takes the
  // hidden-signal toggle's path above for the same reason: the cached
  // window *is* the data this needs, and what has to be rebuilt from it
  // is the conversion, the normalisation and the side list's readout —
  // and those only happen on a resample. Without it the label moved
  // alone: a stopped panel's window never changes, so nothing would
  // ever redraw it, and a running one drew the old scale until its next
  // tick. It is also what carries a *restored* panel's saved choice
  // into the first window it drew, which was fetched before the host
  // answered what that choice converts by.
  const displayUnitKey = signals
    .map((s) => {
      const d = displayUnits.get(signalRefKey(s));
      return d ? `${d.display}:${d.affine.gain}:${d.affine.offset}` : "";
    })
    .join("|");
  const displayUnitKeyPrevRef = useRef(displayUnitKey);
  useEffect(() => {
    if (displayUnitKeyPrevRef.current === displayUnitKey) return;
    displayUnitKeyPrevRef.current = displayUnitKey;
    repaintFromCacheRef.current = true;
    void resampleRef.current();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [displayUnitKey]);
