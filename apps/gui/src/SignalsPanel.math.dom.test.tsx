...
/// `params` is the panel's persisted view config — what a saved layout
/// restores it from ({@link useElementPanel}'s `savedConfig`).
...
describe("a math signal's row", () => {
  ...
  it("wears one bus chip per contributing bus and says so when several do", async () => {
    ...
    // The chips' home is the **message** cell: it is this table's
    // provenance cell (it carries the file-backed badge for the other
    // provenance that has no bus and no message), and it is the
    // analogue of a plot side list's message line, which renders these
    // very chips before this very label. Not an artefact of which
    // columns happen to be shown.
    expect(
      mathRowEl()!.querySelector(".col-msg")!.querySelectorAll(".plot-bus-swatch"),
    ).toHaveLength(1);
  ...
  it("names the contributing buses in the bus cell, and its rate in msg/s", async () => {
    ROWS = [{ ...MATH_ROW, rate: 9.5 }, PLAIN_ROW];
    MATH = [mathRecord({ busIds: ["p", "z"] })];
    // Both columns are hidden on a fresh panel; a saved layout shows
    // them, and that is the layout this pins.
    renderPanel({
      columns: [
        { key: "bus", width: 100, visible: true },
        { key: "rate", width: 64, visible: true },
      ],
    });
    await waitFor(() => expect(mathRowEl()).not.toBeNull());
    // A bus column names buses. The message label ("Math - Multiple
    // Busses") belongs to the message cell, and repeating it here was
    // what made the chips' placement read as an accident.
    const bus = await waitFor(() => {
      const cell = mathRowEl()!.querySelector(".col-bus")!;
      expect(cell).toHaveTextContent("Powertrain, Zonal");
      return cell;
    });
    expect(bus).not.toHaveTextContent("Math");
    // The cadence its operands gave it, served by the host
    // (`select_math`) like every other row's — the column is no longer
    // blank for a computed series.
    expect(mathRowEl()!.querySelector(".col-rate")).toHaveTextContent("9.5");
  });
