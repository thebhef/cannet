...
Frame encoding is imported rather than reimplemented:
`cannet_python_wire` holds the checked-in gencode and the
...
import grpc
from cannet_python_wire import (
    PROTOCOL_PACKAGE,
    Frame,
    frame_to_message,
    frame_to_proto,
...
class IncompatibleProtocol(SessionError):
    """The server does not serve the protocol package this client speaks.

    Terminal (ADR 0059): a breaking wire change is a *new package*, and
    no amount of retrying adds one to a running server. Either this
    client migrates, or the server serves the old major beside the new
    one for its deprecation window.

    Raised before the ``Session`` stream is opened and before any
    credential is presented, so a client that cannot talk to a server
    at all learns *that* rather than that its token is wrong.
    """

    def __init__(self, served: Sequence[str]) -> None:
        self.served = list(served)
        if self.served:
            super().__init__(
                f"serves {', '.join(self.served)}; "
                f"this client speaks {PROTOCOL_PACKAGE}"
            )
        else:
            super().__init__(
                f"serves no protocol packages; this client speaks {PROTOCOL_PACKAGE}"
            )
...
def server_info(
    ...
    """Ask ``channel`` what server is on the other end of it.

    ``ServerInfo`` lives in its own unversioned package and is answered
    without a credential (ADR 0059), so this works against a server
    serving a major this client knows nothing about — which is the one
    case it exists for.
    """
    return info_pb_grpc.CannetInfoStub(channel).ServerInfo(
...
def check_protocol(
    ...
    """Refuse ``channel`` unless the server serves :data:`PROTOCOL_PACKAGE`.

    Called before the ``Session`` stream is opened, so an incompatible
    server is a sentence naming both sides rather than an opaque
    ``UNIMPLEMENTED`` on the first real call.

    A server that does not implement ``ServerInfo`` at all is refused
    the same way: it predates the rule, so nothing it answers can be
    read as an agreement about the wire.
    """
    try:
        served = list(server_info(channel, target, timeout).packages)
    except grpc.RpcError as exc:
        if exc.code() is grpc.StatusCode.UNIMPLEMENTED:  # type: ignore[attr-defined]
            raise IncompatibleProtocol([]) from exc
        raise
    if PROTOCOL_PACKAGE not in served:
        raise IncompatibleProtocol(served)
...
class Session:
    ...
    def __init__(
        ...
        *,
        ...
        # Before anything else on this channel: a server that does not
        # serve this client's protocol major is refused here, with a
        # sentence, instead of failing somewhere downstream (ADR 0059).
        try:
            check_protocol(self._channel, target, timeout)
...
def _proto_to_message(proto_frame: pb.Frame) -> Any:
    """Wire ``Frame`` -> ``can.Message``, through the shared mappers.

    ``frame_to_message`` is the transmit direction, where a timestamp
    and a direction have no meaning, so it sets neither; both are the
    point of a received frame and are applied here. The wire carries
    Unix-epoch nanoseconds and python-can carries Unix-epoch seconds as
    a float.
    """
...
def _message_to_proto(message: Any) -> pb.Frame:
    """``can.Message`` -> wire ``Frame``, through the shared mappers.
