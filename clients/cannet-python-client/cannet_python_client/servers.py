...
class ServerRow:
    ...
    #: ``proto`` TXT key (ADR 0059). ``None`` when it advertised none,
    #: which is "did not say" — the connection's own ``ServerInfo``
    #: call is what actually gates it.
...
def protocol_label(row: ServerRow) -> str:
    """The protocol column: what the server says it speaks.

    ``-`` when it advertised nothing — an older build, a server started
    ``--no-mdns``, an address added by hand. A package set that does not
    hold this client's is called out, because the row is otherwise a
    server that looks reachable and refuses every connect
    (ADR 0059).
    """
    if not row.protocols:
        return "-"
    served = ", ".join(row.protocols)
    if PROTOCOL_PACKAGE in row.protocols:
        return served
    return f"{served} (not spoken here)"
