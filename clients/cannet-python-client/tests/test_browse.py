...
def test_the_proto_key_is_read_as_the_packages_the_server_serves() -> None:
    # ADR 0059's advisory half: the packages a server says it serves,
    # before anything dials it.
    assert browse.parse_protocols("cannet.v1") == ["cannet.v1"]
    assert browse.parse_protocols("cannet.v1,cannet.v2") == [
        "cannet.v1",
        "cannet.v2",
    ]
    # Whitespace around a value is the kind of thing a third-party
    # responder does; it must not become a package nobody serves.
    assert browse.parse_protocols(" cannet.v2 , ") == ["cannet.v2"]


def test_a_proto_key_that_names_nothing_is_the_same_as_no_key_at_all() -> None:
    # "Did not say" and "serves nothing" are different answers, and a
    # blank value is the first.
    assert browse.parse_protocols(None) is None
    assert browse.parse_protocols("") is None
    assert browse.parse_protocols(" , ") is None
