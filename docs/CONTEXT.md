...
is a state the user could be left in and repair. A fixed-arity
function's operands are **named slots** — A and B for a difference —
and an empty one stays empty: filling B does not make that operand A.
