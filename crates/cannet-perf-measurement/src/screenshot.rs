...
///
/// Kept in its own `.js` file rather than inline: every selector here
/// is a claim about the app's markup, and a claim nothing checks rots
/// silently (`importIdle` once returned true mid-import because the
/// label it polled for had been restyled away). As a file it is loaded
/// and driven against the real components by the frontend suite
/// (`shotPrelude.dom.test.tsx`), which fails when a selector stops
/// matching.
const PRELUDE_JS: &str = include_str!("shot-prelude.js");
