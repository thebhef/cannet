...
Status: Slice 0 shipped. Slices 1-4 are **Phase 16** of
`plans/phased-implementation.md`; the indefinite-length / disk-spill
work they are designed against is **Phase 17**.
...
they are now scheduled into Phases 16 and 17. Domain terms — capture,
...
- The raw frame store cannot be RAM-resident. At 10^9 frames it spills
  ...
  *not* bounded scrollback — no historical row is ever evicted or made
  unreachable. This is Phase 17; see
...
Phase 17 is a second implementation behind the same surface, not a
redesign.
...
land against the current in-RAM `TraceStore` `Vec`; Phase 17 swaps the
...
signatures (random-access, async, paged) — so Phase 17 is a second
...
- `RowPage` and `DecimatedRange` are defined as frozen signatures,
  ...
  reimplemented over the disk-spilled store in Phase 17;
...
at `Vec` scale and is what Phase 17's filter index makes O(page) at
10^9 frames.
...
`SignalCacheStore`; Phase 17 gives that projection its decimated
...
- **Disk-spill itself.** Slices 1-4 land against the in-RAM `Vec`; the
  random-access disk-spilled store is Phase 17, behind the
  Slice-1-frozen contract.

## Priority and sequencing

This is **Phase 16** of `plans/phased-implementation.md`. The
...
disk-spill work is a **model-side** change (**Phase 17**). They meet
only at the host accessor signatures, which Slice 1 freezes. So Slices
1-4 land first, against the in-RAM `Vec`; Phase 17 then provides a
