...
- **Task 158 phase 3 — the Events panel's bus-error section pages by a
  growing point budget over `[sessionStart, now)`, not by index.** The task
  text named the trace/by-id hooks as reference adapters for
  `useWindowedQuery`; those page a real host-index-addressable row space,
  which `bus_error_series` doesn't have (time-and-budget windowed only). The
  section instead reuses `useWindowedQuery`'s fetch *lifecycle* only, with a
  budget that doubles (100 → …→ capped at 4000) when the reader scrolls to
  the section's bottom edge — exact (every row a real served point, no
  interpolated offset→time guessing) and bounded, but not true random-access
  paging (no jump-to-arbitrary-row; Home/End move within whatever's
  currently loaded). Detail: `plans/tasks/0158-bus-error-markers-page.md`
  phase 3 status log, "A design decision recorded" paragraph.
...
- **158 — bus-error markers page** (3 phases, `task158-error-series`
  → `task158-plot-markers` → `task158-events-section`): 7/7 exit
  criteria met. The Events section pages by a growing point budget
  over the whole capture rather than by time window — a § 1 item.
