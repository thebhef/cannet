...
Recorded by the phases that found them, not yet decided. **64 findings, 51 open**
(group I walked 2026-08-25; 3.63 added and 3.64 opened and closed 2026-08-26).
...
| ~~C~~ | **What you saw on the bench** | 3.37, 3.39, 3.42, 3.43, 3.53 | **Walked 2026-08-26 — all five scoped to [task 121](tasks/0121-the-trace-tells-the-truth-about-the-wire.md)**, with 3.39 ruled: error frames stay in the saved capture and coalesce in the frontend. 3.42 and 3.53 still need a ruling inside that task. **Two of the five are the owner's own observations** — your 2026-08-23 session. 3.37 and 3.43 are one root: a tx-confirm row is a local echo, never a wire confirmation. 3.39 is 5,200 error frames/s reaching the trace unfiltered. |
...
**3.63 is ungrouped** — found while closing group I, so it belongs to no group drawn before it existed. (3.64 was opened and closed the same day.)
...
| ~~3.5~~ | **Shipped 2026-08-26.** Eleven genuinely broken links repaired. The other 41 were first allowed at the crate root, then **fixed instead** on 2026-08-26 when the owner ruled the gate should cover everything — see 3.64. Gated at `-D warnings` across the **workspace** in the commit hook, in CI (a seventh job), and in both agent skills. **Ruled 2026-08-25 — fix them and gate them: *"fix it, add to pre-commit."*** `cargo doc -p cannet-gui` emits **47 warnings** — unresolved links, public docs pointing at private items. All pre-existing, none from this chain. No task opened. | shipped |
...
| ~~3.64~~ | **Closed 2026-08-26 by owner ruling — *"Include library crates - it should be everything."*** The gate is `--workspace` in the commit hook, in CI and in the agent skills, and every crate is clean. **All 75 warnings are fixed, none allowed:** the 65 `private_intra_doc_links` were de-linked (the name stays, in code font, so the text still reads and no visibility widened), 8 unresolved links repaired, and 2 module/macro ambiguities disambiguated with `mod@`. The crate-root `allow` that had made cannet-gui clean is **removed**. | shipped |
