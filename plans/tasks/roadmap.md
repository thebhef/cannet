...
1. [Task 62 — Solo Filter Redesign](0062-solo-filter-redesign.md)
   — from owner live use 2026-08-09: solo scoped to areas with
   matches (zero-match areas untouched), one regex dialect with the
   area patterns (full path, case-sensitive), group-by-capture
   stepping (`Cell(\d+)` steps by cell index; numeric-aware ordering,
   `$N` ordinal override in named groups, tuple keys) paged by
   `solo_page_size` with All in the cycle, and four visibility
   surfaces (distinct solo styling, per-area match chip,
   two-form/no-matches readout, click-to-open match menu).
2. [Task 61 — Ingest Perf Round 2](0061-ingest-perf-round-2.md)
   — the two data-named cuts from task 58's ingest profiling: the
   disk-spill segment write (43 % of the release per-frame budget)
   and `bus_id: Option<String>` interning (~15 %). Opened by owner
   ruling 2026-08-09.
3. [Task 41 — Production Cannet Server](0041-production-cannet-server.md)
...
4. [Task 42 — Server Connection Security](0042-server-connection-security.md)
   ...
   `--insecure`.
5. [Task 43 — Server Discovery](0043-server-discovery.md)
...
6. [Task 31 — macOS Integration Issues](0031-macos-integration-issues.md)
...
7. [Task 19 — Argument-Taking Palette Commands](0019-command-palette-goto.md)
...
8. [Task 25 — CAN HW + Virtual-Bus Bug Fixes](0025-can-hw-vbus-bugfixes.md)
