...
1. [Task 63 — Plot Usage-Feedback Round](0063-plot-usage-feedback.md)
...
6. [Task 38 — MDF (MF4) Logger-File Import](0038-mdf-import.md)
    ...
    MF4 is explicitly later; export follows as Task 64.
7. [Task 64 — MDF (MF4) Capture Export](0064-mdf-export.md)
   — export a capture to ASAM MDF 4.x so MDF round-trips with Task
   38's import (vendor-neutral, and MDF's AT attachment / MD blocks
   fit ADR 0010's in-file embedding rule). Needs grooming first;
   user-provided example MDF data exists with message-independent
   signals, a concept the model must grow by the time MDF support is
   done.
