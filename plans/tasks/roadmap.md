...
1. [Task 32 — Plot Vertical Layout: Fit-to-Panel Weights, Splitters, Enum Lanes](0032-plot-vertical-layout-enum-lanes.md)
   — fit-to-panel weighted axis heights (no stack scrolling),
   splitter drag between plot areas, and a compressed per-unit enum
   view collecting all of an area's enums onto one stacked-lane
   axis. Absorbs two Task 23 notes (vertical area resize, enum
   packing).
