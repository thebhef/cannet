...
- 2026-09-16 — owner accepted as-is: the project-directory-mode
  reading (`!ProjectDir::is_auto_located()`), and loggers writing
  both directions ("a file's frame count should include TX
  messages").

### 2026-09-17 — owner-authored fix absorbed: the first export's picker (branch `task137-export-dialog`)

Owner fix folded in here rather than landed on top: on a fresh install
the host *omits* `folder` from `get_export_state` (serde skips a `None`)
so the frontend sees `undefined`, which the `folder === null` test in
`App.tsx` missed — `joinExportPath` was called with no folder and the
save picker never opened, so the very first export of a fresh install
did nothing. `== null` covers both spellings, `ExportStateRecord.folder`
is now optional (`folder?: string | null`) to match the real wire shape,
and `App.saveCapture.dom.test.tsx` gained a case ("opens the picker
before any export has remembered a folder") that pins the picker opening
seeded with just the file name; verified red-first on this branch
(TypeError in `joinExportPath`, no `save` call) before the fix.
