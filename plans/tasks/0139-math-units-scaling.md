...
**2026-09-15 — owner defect: a picked display unit relabelled but
rescaled nothing.** Folded into `task139-apply`, the branch that
introduced the plot's convert affordance.

*Observation (bench, installer off `47eaf635`):* choosing a different
unit on a plot series' readout chip changed the label and moved the
series onto the target unit's lane, but the plotted values, the y
axis and the side list's readout all stayed at the declared scale.

*Hypothesis 1:* nothing forces a redraw on a display-unit change, so a
stopped panel — whose window never moves — never applies the affine.
*Experiment:* a DOM repro on a stopped panel (`trace: { start: 0, end:
60 }`), one `mV` series, pick `V`, assert the readout. *Data:* it
converted, 3712 → 3.712. *Refuted as stated* — but the repro had picked
while the mount's own one-shot re-samples were still landing.

*Hypothesis 2 (the refinement that held):* the affine is applied only
inside the re-sample's "sampled" branch, read from a ref, so whether a
pick takes effect depends on a re-sample happening **after** the host's
answer arrives. *Experiment:* the same repro, with the panel driven to
quiescence (two windows with no `sample_signals` call) before the pick,
and the `sample_signals` count probed either side of it. *Data:* the
readout stayed `3712` while the chip read `V`; the count went 1 → 2,
so a re-sample did run — the one the pick's own re-render caused, which
fired while `displayUnitsRef` still held the pre-pick answer. The map
arrived one round-trip later and nothing redrew after it.
*Conclusion, confirmed:* the conversion had no trigger of its own. A
running panel hid it — its next tick (~70 ms) redrew through the new
affine — which is why the phase's verification and the existing
lane-merge test, both against a running trace, passed.

*Fix.* A display-unit change is a view-only change over data already in
hand, exactly like a `hidden` toggle or a manual y bound — and each of
those already repaints from the cached window. The third one now does
too: a key over this area's `(display, gain, offset)` triggers a
repaint-from-cache re-sample, so no round-trip is made and the
normalisation, the readout and the axis label are all rebuilt through
the new affine. It also carries a *restored* panel's saved choice into
its first drawn window, which was fetched before the host answered what
that choice converts by.

The host's all-time extent is now cached **as the host served it** and
carried into the drawn unit where the range is read, rather than
converted on the way into the cache. A repaint reuses that cached
answer, and an extent already carried through the old factor cannot be
carried through the new one — it would have left a following panel's y
axis on the old scale for exactly as long as the values were on the new
one.

Tests: two DOM guards, both written red first and both falsified
against the fix (the extent one goes red when the carry is removed) —
the stopped-panel redraw, which also pins that the conversion costs no
`sample_signals` round-trip, and the following-panel host extent.
Frontend 3463 → 3465 across 246 files.

- 2026-09-16 — owner ruled on the four queued composed-unit /
  ordering items, all approved as landed: alphabetical dimension-group
  order stands (no curated sequence intended); the `W = V * A`
  refusal stands, docs and examples use a free name (`VA = V * A`);
  composed units take no SI prefix — a scaled variant is its own
  definition via a numeric factor; and factors are legal in any
  position, which is also the answer to "can someone put constants in
  their composed units" — yes.
