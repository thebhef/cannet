...
### 2026-09-14 — bench report "a statistic plots over time" — not reproducible, sighting predates the fix

**Report (owner, testing an installer):** a statistic added while the
capture was still running drew as a series over time rather than the
horizontal line it names.

**Observation.** The staircase the report describes is exactly what
`statistic_round` produced before a round began returning the whole
line and replacing the series. That change is in the tree the report
was filed against: `git show 47eaf635:.../signal_cache.rs` contains
`SignalCache::rewrite`.

**Hypothesis 1 — a definition created *mid-capture* escapes the
rewrite.** The existing guard
(`a_statistic_served_while_the_capture_grew_is_still_one_flat_line`)
serves a definition that was in the model before the first frame; the
report's order is the other one, where the operand pyramid is already
several serves old when the definition appears.

*Experiment.* A test in that order:
`a_statistic_defined_mid_capture_is_one_flat_line_that_moves` — three
rounds serving `A` alone with no math in the model at all, then the
definition, then three more rounds of growth, asserting two points, a
flat value equal to the whole capture's max, and a matching
`min_max_many` extent at *every* round.

*Data.* Passes at the tip, unmodified. Hypothesis refuted.

**Hypothesis 2 — the plot's own serve shape exposes it.** The plot
serves through a *bounded* store (a serve budget), over a moving
window, decimated — none of which the guards use.

*Experiment.* A scratch run at production shape: `SignalCacheStore::new`
(budgeted), 2000 frames at 100 Hz, the definition added after ten
serves, then thirty rounds each serving `[newest − 10 s, newest + 1 s]`
at `max_points = 800` and re-reading the raw series.

*Data.* Every round: raw series two points, window `[(0.0, v),
(t_newest, v)]` — flat, and anchored at the capture start even though
the window begins later. Hypothesis refuted. (Scratch test not kept;
it asserts nothing the two guards do not.)

**Hypothesis 3 — the frontend accumulates.** Falsified by reading:
`useDecimatedRange` replaces its points on every fetch and holds no
cross-poll array, and the only live-tail overlay in the app
(`liveTailDemand`) is the trace's, not the plot's.

**Hypothesis 4 — the fix is present but was not in the build the owner
ran.** The owner's follow-up: "it's entirely possible I haven't checked
it since 9-08."

*Experiment.* Walk `git reflog show task135-surfaces` and ask each entry
whether it carries `fn rewrite`.

*Data.* `b7f9a44d` (2026-09-08 19:13) does not; `a64e9c2c`
(2026-09-08 19:33) is the first that does. The branch settled at
`6b361376`, committed 2026-09-09 23:56, and the close-out tip the
installer was built from was committed 2026-09-10 01:28.

**Conclusion.** The defect is real and is already fixed; the sighting
is from a build cut before 2026-09-08 19:33 PDT. Confirmed
discriminating rather than assumed: with `fill.constant` forced false,
the new mid-capture test fails with
`[(0.0, 119.0), (119.0, 119.0), (0.0, 149.0), (149.0, 149.0)]` — the
staircase, in the report's own shape — and passes again when the
rewrite is restored. Nothing changed but the test, which is kept on
`task135-surfaces` beside the guard it complements, because it pins the
*order* the bench hit rather than only the failure mode.

### 2026-09-15 — bench fix: a redefinition did not reach the plot (`task135-engine`)

Owner, on the installer cut from `47eaf635`: *"updating a math signal
does not result in any changes to the plotted signal."* Editing an
existing definition while its series is on a plot left the plotted
series exactly as it was.

**Observation.** Edit an existing math definition (gain, operand,
parameter, unit) with the series on a plot; the drawn series does not
move. Reported as happening on a running panel too, which rules out a
missing repaint on a stopped one.

**Hypothesis 1.** The host serves the stale pyramid — the compositional
fingerprint does not cover the edited field, or the invalidation misses
the redefinition case.

**Experiment 1** (`signal_cache`, scratch): serve `Scale{gain: 1}` over
a decoded operand, swap the definition to `Scale{gain: 2}`, call
`invalidate_dbcs` with the new model, serve again.
*Data.* First serve `[0, 1, 2, 3, 4 …]`; second serve `[0, 2, 4, 6,
8 …]`. **Refuted** — the store recomputes.

**Experiment 2** (`AppState`, the real invalidation path): define the
same signal through the registry, serve, `MathRegistry::update` to gain
2, `invalidate_derived_caches`, serve again.
*Data.* `[0…9]` then `[0, 2, 4 … 18]`. **Refuted again** — the whole
host path recomputes, including the resolved-model rebuild. Kept as
`a_math_redefinition_rebuilds_what_the_host_serves`.

**Hypothesis 2.** The host recomputes but never says so, so no view
asks again.

**Experiment 3** (source, falsifiable by a single counter-example):
enumerate every host path that calls `invalidate_derived_caches` and
check which announce.
*Data.* `dbc_commands` (add / re-scope / remove / clear),
`dbc_watcher`, `capture`'s embedded install, `signal_units`'s
reinterpretation and `view_signals`'s per-signal pick all call
`announce_dbc_change`. Two do not: `math_commands::changed` and
`lib::math_model_needs_rebuilding`. **Confirmed.**

**Root cause.** `math_commands::changed` emitted only
`math-signals-changed`. That event is consumed by the math *listing*
(`mathSignalsContext`) and by nothing else. A plot's samples are cached
against the trace model's re-anchor epoch, whose only carrier is
`dbc-changed` (ADR 0053 §2/§3) — so the recomputed series was never
fetched, on a running panel as much as a stopped one: the live tail
extends the cached window rather than refilling it. ADR 0053 §2 names
this exact shape a defect: *"a host path that changes the set and stays
quiet is a defect."*

**Fix.** A registry write announces both facts. The pair goes through a
small in-crate sink trait (`RegistryChangeSink`) because Tauri's
`Emitter` is sealed and the announcement otherwise has no test seam —
which is why the missing half shipped.

**Why earlier verification missed it.** Both halves were tested, and
neither test could see the gap. Host-side tests drove
`invalidate_derived_caches` directly and proved the recompute;
frontend-side, `PlotPanel.dom.test.tsx` proves a plot re-anchors when
`dbc-changed` arrives. Nothing tested the join — that a math write is
one of the paths which emits it. The new test is that join.

**The same gap on the other input, fixed the same day.** The audit's
second quiet path — `lib::math_model_needs_rebuilding`, the
unit-customization side — is closed under the owner's narrow ruling;
see the first entry under Blockers / side effects for the shape and the
list that has to grow with the inputs.

**Tests.** 2 added on `task135-engine` (`tests.rs`):
`a_math_registry_write_announces_the_listing_and_the_decode_model`
(red before the fix: `["math-signals-changed"]` against
`["math-signals-changed", "dbc-changed"]`) and
`a_math_redefinition_rebuilds_what_the_host_serves` (Experiment 2,
green before and after — it pins the half that was already right).
`cargo test -p cannet-gui` 1126 pass on the branch, 1280 at the stack
tip.

**A tauri mock-runtime seam was tried first and abandoned.** Enabling
`tauri`'s `test` feature as a dev-dependency builds, but the resulting
test binary will not load on Windows (`STATUS_ENTRYPOINT_NOT_FOUND`,
0xc0000139 — every import resolves, so it is a comctl32 v5/v6 manifest
mismatch the app binary's generated manifest covers and a test binary's
does not). The whole crate's test binary dies, not just the new test,
so the feature is not usable here. The in-crate trait is the
alternative.

- 2026-09-16 — owner walked the queue. Accepted as-is: an undone
  deletion returning at the end of the Computed branch, unfinished
  definitions being stored, and the leading disclosure ("we don't
  have an existing 'expand my plot signal header' anywhere").
  Ordered on this stack: the pair-pick slot form (a pick into B
  while A is empty must land in B), a math row's rate and chip
  placement in signal views, and a standing project-bus map on
  `AppState`. On session-scoped pyramids: 135-3 landed and its math
  case measured the steady-state cost (table above); a dedicated
  reopen-recompute number is still owed at close-out.
...
- ~~**The same defect remains on the unit-customization path.**~~
  **Fixed 2026-09-15, narrow shape** (ruled: the unconditional announce
  is rejected). `lib::math_model_needs_rebuilding` is gone;
  `settings::set_settings` reads `unit_inputs()` before the write,
  compares it after, and calls `settings::apply_unit_change` — which
  routes through `math_commands::changed`, so a moved input drops the
  resolved model, re-judges the pyramids and announces the
  decode-model change, and an unmoved one announces nothing. A theme or
  poll-interval save therefore re-anchors nothing.

  **`unit_inputs()` is the list to extend.** It is the merged
  customization dict and the merged composed-unit definitions, both
  scopes — everything a conversion factor is read from. A new input
  left out of it reproduces this defect exactly: the host rescales and
  no plot refetches. It grows on the branch that adds the input
  (`task139-derive` added user-scope customizations to it,
  `units-composition` added the definitions), so the list and the input
  land together.

  Three tests, on the branches that own each half: a customization move
  announces both events, a composed-unit move announces both, and a
  write across which nothing unit-related moved announces nothing. Both
  directions were falsified — dropping the `changed` call empties the
  recorder on the first two, and making the gate unconditional fires the
  third.

- ~~**Bus names reach the host only through a math command.**~~
  **Fixed 2026-09-16** (owner-ordered), on `task135-engine`: the
  project's bus list is standing state on `AppState`
  (`project_bus_names`, written only by `set_project_bus_names`),
  installed by `open_project` from the file it parsed, cleared by
  `close_project`, and refreshed by `rbs_sync_project_buses` — the push
  the frontend already makes on every bus add / rename / remove. The
  per-call carry is gone with it: no math command takes `busNames`, so
  a name-anchored pattern matches on the first serve after an open.
  `fetch_signal_page` and `list_view_signals` still take the map per
  call; that is their own sort/labelling concern and was left alone.
  The status-log entry above has the walk.

  **Residue for the frontend phases:** the call sites added by 135-2
  and 135-3 passed `busNames` to `list_math_signals`,
  `define_math_signal` and `update_math_signal`. Tauri reads command
  arguments by key, so the extra key was never read and nothing broke —
  but it is a key with no reader, and the branch that owns each call
  site drops it (including the `mathDefine` / `mathUpdate` edit history
  records that carry it). **135-2's are gone** (`task135-editor`, the
  slot entry above). **135-3's are gone too** (`task135-surfaces`, the
  rate/chips entry above): `mathSignalsContext.tsx` passes nothing,
  publishes no `mathBusNames` and says so, and the `--math-on-start`
  harness path stopped carrying the key into `define_math_signal`. No
  math command takes the map on any branch of this stack.
...
5. **Met** (135-3): drag sources everywhere, per-bus chips + "Math - Multiple Busses", statistic as capture-constant hline,
   hline solid (ADR 0026 amended), MDF export carries the Computed group (oracle extended), BLF none. The blank rate column and
   the chips' placement were **resolved 2026-09-16** on owner order (the entry above): a math row's `msg/s` carries the cadence
   the host derived from its operands, and the chips' home in the message cell is the plot-side-list analogue rather than an
   artefact of which columns show. Minor queued residue: disclosure placement vs prototype.
