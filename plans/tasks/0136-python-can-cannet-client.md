...
`python-wire` CI job re-runs `scripts/regen_proto.sh` — already the
contributor path for editing `cannet.proto` — and
...
- 2026-09-16 — owner walked the queue: the TLS e2e gap moves to a
  task (new, or an existing one targeting that area) rather than this
  stack; the ruff lock divergence gets fixed on the stack.
...
- 2026-09-18 — the sidecar `SMOKE.md` `plans/backlog.md` reference
  (queue §5, standing instruction: scrub with the next
