...
which case that is the one named and the one that decodes). A
**multiplexed** message groups
...
- **The toolbar's right-click menu.** Right-click the plot toolbar for
  ...
  frames in the trace window, cached plot points — off by default), and
  the sources picker.
...
bare 0–1 scale (`ratio`). Both ship as default spellings; a project
...
`Math - Multiple Busses` when more than one does. In a signal view the
chips sit in the **message** column, the same line a plot's side list
puts them on; the **bus** column names those buses, and **msg/s** reads
the cadence the operands gave the computed series (an `HLine` or a
`Statistic` has none — it is one value across the capture, not a series
sampled over it). The row's disclosure
